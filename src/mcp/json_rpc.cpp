...
std::optional<JsonRpcRequest> JsonRpcParser::parseRequest(
  ...
  // Step 5: extract id (string, number, or null).  A missing id is a valid
  // Notification per JSON-RPC 2.0 and MCP ("notifications/initialized" is
  // typically sent without an id); it maps to a null id so isNotification()
  // reports true and no response is written.
  if (!root.hasMember("id")) {
    request.id = JsonValue();  // null -> Notification
  } else {
